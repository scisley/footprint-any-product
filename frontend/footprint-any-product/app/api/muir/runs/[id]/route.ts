...
export async function GET(
  ...
  try {
    ...
    // Execute the Python script to get run by ID
    const { stdout, stderr } = await execPromise(`python3 ../../../../../frontend/muir_api_client.py get_run_by_id ${runId}`);
    ...
    for (const line of outputLines) {
      if (line.includes(`Carbon Run Details for Run ID ${runId}:`)) {
        captureJson = true;
        continue;
      }
