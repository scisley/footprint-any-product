...
export async function GET() {
  try {
    // Execute the Python script to get organization info
    const { stdout, stderr } = await execPromise('python3 ../../../../frontend/muir_api_client.py get_info');
    ...
    // We need to find the actual JSON part in the output
    let jsonDataString = '';
    const outputLines = stdout.split('\n');
    let captureJson = false;
    
    for (const line of outputLines) {
      if (line.includes('Organization Info:')) {
        captureJson = true;
        continue;
      }
