...
export async function GET(request: Request) {
  try {
    ...
    // Execute the Python script to get procurement IDs
    const { stdout, stderr } = await execPromise(`python3 ../../../../frontend/muir_api_client.py get_procurement_ids ${page} ${perPage}`);
    ...
    for (const line of outputLines) {
      if (line.includes('Procurement IDs:')) {
        captureJson = true;
        continue;
      }
